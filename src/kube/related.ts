...
import { discoverResourceTypesCached, listResources, resourceBasePath } from './client';
...
// MARK: Cross-references beyond ownership

/**
 * Non-ownership relations for the detail view: Service → selected Pods,
 * Ingress → backend Services, PVC ↔ volume/mounting Pods, Pod → PVCs.
 * Ownership chains (Deployment → ReplicaSets → Pods …) are handled by
 * listOwnedResources above.
 */
...
export async function findRelatedResources(
  ...
  if (
    typeKey !== '/Service' &&
    typeKey !== 'networking.k8s.io/Ingress' &&
  ...
  const childList = async (
    ...
    options: { labelSelector?: string; limit?: number },
  ...
  if (typeKey === '/Service') {
    await childList('Pods', typeFor('', 'Pod'), { labelSelector: selectorOf(spec.selector) }, undefined, podNote);
