...
import { parseCpu, parseMemory, formatCores, formatGiB } from '../../kube/quantity';
import {
  CLUSTER_CPU_QUERY,
  CLUSTER_MEM_QUERY,
  getFiringAlerts,
...
import { formatCpuUsage, formatMemoryUsage } from '../../kube/metrics';
import { useClusters } from '../../state/ClustersContext';
import { useClusterNav } from '../../state/ClusterNav';
import { ApiResourceType, ClusterConfig } from '../../types';
import { Card, HealthRing, Sparkline, StatusDot, UsageBar } from '../kit';
import { AlertSheet, ClusterSwitcherSheet, NamespaceSheet, SettingsSheet } from '../sheets';
import { colors, radius, spacing } from '../theme';
import { EmptyState, ErrorBox, Loading } from '../components';
import { useResponsiveLayout } from '../useResponsiveLayout';
import { ageOf } from '../../util/format';

const POD_TYPE: ApiResourceType = { group: '', version: 'v1', plural: 'pods', kind: 'Pod', namespaced: true, verbs: ['get', 'list', 'update', 'delete'] };
const NODE_TYPE: ApiResourceType = { group: '', version: 'v1', plural: 'nodes', kind: 'Node', namespaced: false, verbs: ['get', 'list'] };
...
export function IndexContent({ clusterId }: { clusterId: string }) {
  ...
  const reloadProm = useCallback(async () => {
    const current = getById(clusterId);
    if (!current) return;
    try {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        {/* On iPad/wide the sidebar already carries the cluster switcher, so
            this header pill would be a duplicate. */}
        {!isWide ? (
        ...
        >
          ...
          <View style={styles.statRow}>
            <StatCard
              ...
              label="Pods"
              onPress={() =>
                nav.show({ kind: 'list', type: POD_TYPE })
              }
            />
            <StatCard
              abbr="No"
              ...
              label="Nodes"
              onPress={() =>
                nav.show({ kind: 'list', type: NODE_TYPE })
              }
            />
            <StatCard
              abbr="!"
            ...
            <>
              <View style={styles.sectionRow}>
                ...
                {prom.alerts.length > 6 ? (
                  <TouchableOpacity onPress={() => nav.show({ kind: 'alerts' })}>
          ...
          <View style={styles.sectionRow}>
            ...
            <View style={{ flex: 1 }} />
            <TouchableOpacity onPress={() => nav.show({ kind: 'events' })}>
              <Text style={styles.viewAll}>View all</Text>
            </TouchableOpacity>
          </View>
